...
from dedupe._api import Dedupe
...
def readData(filename):
    ...
    with open(filename) as f:
        ...
        for row in reader:
            clean_row = [preProcess(v) for v in row.values()]
            clean_row[0] = int(clean_row[0])
...
if __name__ == '__main__':
    ...
    output_file = '_csv_example_output.csv'
    settings_file = '_csv_example_learned_settings'
    training_file = '_csv_example_training.json'
    ...
    if False and os.path.exists(settings_file):
        ...
        #     ]
        fields = [
            VariableDefinition(field_index=2, type='String'),
            VariableDefinition(field_index=3, type='String'),
            VariableDefinition(field_index=4, type='Exact', has_missing=True),
            VariableDefinition(field_index=5, type='String', has_missing=True),
        ]
        ...
        print(f"prepare_training() took {time.time() - t}")
        # ## Active learning
        ...
        t = time.time()
        deduper.train()
        print(f"train() took {time.time() - t}")
    ...
    print(f"clustering took {time.time() - t}")
    ...
    with open(output_file, 'w') as f_output, open(input_file) as f_input:
        ...
        fieldnames = ['Cluster ID', 'confidence_score'] + reader.fieldnames  # type: ignore[operator]
