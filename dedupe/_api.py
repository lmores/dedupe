...
import dedupe._blocking as blocking
import dedupe._clustering as clustering
import dedupe._core as core
import dedupe._datamodel as datamodel
import dedupe._labeler as labeler
import dedupe._predicates
import dedupe._serializer as serializer
from dedupe.__typing import TrainingData
...
if TYPE_CHECKING:
    ...
    from dedupe.__typing import (
        ...
        LookupResults,
        Record,
        RecordPair as TrainingExample,
        RecordPairsList as TrainingExamples,
        RecordID,
        RecordPairs,
        Scores,
        VariableDefinition,
...
class Matching(object):
    ...
    def __init__(
        ...
        self.predicates: Collection[dedupe._predicates.Predicate]
...
class IntegralMatching(Matching):
    ...
    def score(self, pairs: RecordPairs) -> Scores:
        ...
        try:
            ...
            raise RuntimeError(
                "You need to either turn off multiprocessing or protect the "
                "calls to the Dedupe methods with `if __name__ == '__main__'` "
                "in your main module, see "
...
class DedupeMatching(IntegralMatching):
    ...
    def partition(
        ...
        clusters = self._add_singletons((d[0] for d in data), clusters)
    ...
    def pairs(self, data_1: Data, data_2: Data) -> RecordPairs:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            con.executemany(
                ...
                self.fingerprinter(data_1),
            )

            con.executemany(
                "INSERT INTO blocking_map_b values (?, ?)",
                self.fingerprinter(data_2, target=True),
            )
...
class GazetteerMatching(Matching):
    def __init__(
        ...
        self.indexed_data: MutableMapping[RecordID, Record] = {}
    ...
    def index(self, data: Data) -> None:  # pragma: no cover
        ...
        self.indexed_data.update((d[0], d) for d in data)  # CRITICAL TODO
    ...
    def unindex(self, data: Data) -> None:  # pragma: no cover
        ...
        for field in self.fingerprinter.index_fields:
            self.fingerprinter.unindex(
                {record[field] for record in data}, field
            )

        con = sqlite3.connect(self.db)
        con.executemany(
            "DELETE FROM indexed_records WHERE record_id = ?",
            ((d[0],) for d in data),  # CRITICAL TODO
        )
        ...
        for d in data:
            del self.indexed_data[d[0]]  # CRITICAL TODO
    ...
    def blocks(self, data: Data) -> Blocks:
        ...
        con.executemany(
            "INSERT INTO blocking_map VALUES (?, ?)", self.fingerprinter(data)
        )
    ...
    def _format_search_results(self, search_d: Data, results: Links) -> LookupResults:
        ...
        #     yield k, ()
        for d in search_d:  # CRITICAL TODO
            if d[0] not in seen:
                yield d[0], ()
...
class ActiveMatching(Matching):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.training_pairs = TrainingData()  # {"distinct": [], "match": []}
        self.classifier = sklearn.model_selection.GridSearchCV(
            #estimator=sklearn.linear_model.LogisticRegression(),
            estimator=sklearn.linear_model.SGDClassifier(loss='log_loss'),
            # param_grid={"C": [0.00001, 0.0001, 0.001, 0.01, 0.1, 1, 10]},
            # TODO: find actual conversion rule for "C" -> "alpha"
    ...
    def _read_training(self, training_file: TextIO) -> None:
        ...
        # Convert old format to new  TODO: avoid/remove
        dvalues, mvalues = None, None
        if training_pairs.distinct:
            if isinstance(training_pairs.distinct[0][0], dict):  # noqa
                dvalues = []
                for p in training_pairs.distinct:
                    dvalues.append((tuple(p[0].values()), tuple(p[1].values()))) # type: ignore
        if training_pairs.match:
            if isinstance(training_pairs.match[0][0], dict):  # type: ignore
                mvalues = []
                for p in training_pairs.match:
                    mvalues.append((tuple(p[0].values()), tuple(p[1].values()))) # type: ignore

        if dvalues is not None or mvalues is not None:
            training_pairs = TrainingData(distinct=dvalues, match=mvalues) # type: ignore
    ...
    def mark_pairs(self, labeled_pairs: TrainingData) -> None:
        ...
        if self.active_learner:
            ...
            logger.info("Someone wanted to throw an exception here !!!")
            pass  # throw???
    ...
    def _checkTrainingPairs(self, labeled_pairs: TrainingData) -> None:
        ...
        #     )
        if not isinstance(labeled_pairs, TrainingData):
            raise ValueError("labeled_pairs must be an instance of TrainingData")
    ...
    def _checkRecordPair(self, record_pair: TrainingExample) -> None:
        ...
        if not isinstance(record_pair[0], Sequence) or not isinstance(record_pair[1], Sequence):
            raise ValueError(
                "A pair of record_pairs must be made up of two Sequences"
            )
...
class Link(ActiveMatching):
    ...
    def _checkData(self, data_1: Data, data_2: Data) -> None:
        ...
        self.data_model.check(data_1[0])
        self.data_model.check(data_2[0])
...
def flatten_training(training_pairs: TrainingData) -> tuple[TrainingExamples, LabelsLike]:
    ...
    match = training_pairs.match
    distinct = training_pairs.distinct

    examples = []
    examples.extend(match)
    examples.extend(distinct)

    y: np.typing.NDArray[numpy.int_] = np.full(len(examples), False)
    y[:len(match)] = True
