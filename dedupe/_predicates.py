...
if TYPE_CHECKING:
    ...
    from dedupe.__typing import Literal, Record
...
class Predicate(abc.ABC):
    ...
    @abc.abstractmethod
    def __call__(self, record: Record, **kwargs) -> Iterable[str]:
        pass
...
class SimplePredicate(Predicate):
    ...
    def __init__(self, func: Callable[[Any], Iterable[str]], field_index: int):
        self.func = func
        self.__name__ = "(%s, %s)" % (func.__name__, field_index)
        self.field_index = field_index

    def __call__(self, record: Record, **kwargs) -> Iterable[str]:
        column = record[self.field_index]
        if column:
...
class StringPredicate(SimplePredicate):
    def __call__(self, record: Record, **kwargs) -> Iterable[str]:
        column: str = record[self.field_index]
        if column:
...
class ExistsPredicate(Predicate):
    ...
    def __init__(self, field_index: int):
        self.__name__ = "(Exists, %s)" % field_index
        self.field_index = field_index

    @staticmethod
    def func(value: Any) -> tuple[Literal["0", "1"]]:
        if value:
            return ("1",)
        else:
            return ("0",)

    def __call__(self, record: Record, **kwargs) -> tuple[Literal["0", "1"]]:
        value = record[self.field_index]
...
class IndexPredicate(Predicate):
    ...
    def __init__(self, threshold: float, field_index: int):
        self.__name__ = "(%s, %s)" % (threshold, field_index)
        self.field_index = field_index
...
class CanopyPredicate(IndexPredicate):
    ...
    def freeze(self, records: Iterable[Record]) -> None:
        self._cache = {record[self.field_index]: self(record) for record in records}
        self.canopy = {}
        self.index = None
    ...
    def __call__(self, record: Record, **kwargs) -> list[str]:
...
class SearchPredicate(IndexPredicate):
    ...
    def freeze(
        self, records_1: Iterable[Record], records_2: Iterable[Record]
    ) -> None:
        self._cache = {
            (record[self.field_index], False): self(record, False) for record in records_1
        }
        self._cache.update(
            {(record[self.field_index], True): self(record, True) for record in records_2}
        )
        self.index = None
    ...
    def __call__(self, record: Record, target: bool = False, **kwargs) -> list[str]:
