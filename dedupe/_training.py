...
if TYPE_CHECKING:
    ...
    from .__typing import ComparisonCover, Cover, Data, Literal
    from .__typing import RecordPairsList as TrainingExamples
    from .__typing import RecordID, RecordIDPair
...
class BlockLearner(ABC):
    ...
    blocker: _blocking.Fingerprinter
    comparison_cover: ComparisonCover
...
class DedupeBlockLearner(BlockLearner):
    def __init__(
        ...
        self.blocker = _blocking.Fingerprinter(predicates)
        self.blocker.index_all(data)

        self.comparison_cover = self.coveredPairs(self.blocker, sampled_records)

    @staticmethod
    def coveredPairs(blocker: _blocking.Fingerprinter, records: Data) -> ComparisonCover:
        cover = {}
        ...
        for predicate in blocker.predicates:
            ...
            for record in records:
                ...
                for block in blocks:
                    pred_cover[block].add(record[0])  # CRITICAL, TODO: pk is assumed to be in the first field
...
class RecordLinkBlockLearner(BlockLearner):
    ...
    def coveredPairs(
        self, blocker: _blocking.Fingerprinter, records_1: Data, records_2: Data
    ) -> ComparisonCover:
        ...
        for predicate in blocker.predicates:
            ...
            for record in records_2:
                ...
                for block in blocks:
                    cover[predicate][block][1].add(record[0])  # CRITICAL, TODO: pk is assumed to be in the first field

            current_blocks = set(cover[predicate])
            for record in records_1:
                blocks = set(predicate(record))
                for block in blocks & current_blocks:
                    cover[predicate][block][0].add(record[0])  # CRITICAL, TODO: pk is assumed to be in the first field
