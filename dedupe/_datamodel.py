...
from dedupe.variables._base import FieldType as FieldVariable
from dedupe.variables._base import MissingDataType, Variable
from dedupe.variables._interaction import InteractionType
...
class DataModel(object):
    ...
    def __init__(self, variable_definitions: Collection[VariableDefinition]):
        if not variable_definitions:
            raise ValueError("Variable definitions can not be empty")
    ...
    def _field_comparators(
        self,
    ) -> Generator[tuple[int, Comparator, int, int], None, None]:
        start = 0
        stop = 0
        for var in self.primary_variables:
            stop = start + len(var)
            # comparator = cast("Comparator", var.comparator)
            yield (var.field_index, var.comparator, start, stop)
            start = stop
    ...
    def predicates(self) -> set[Predicate]:
        ...
        for var in self.primary_variables:
            ...
            predicates.update(var.predicates)
        return predicates

    def distances(self, record_pairs: Sequence[RecordPair]) -> NpFloatArray:
        distances = np.empty((len(record_pairs), len(self)), "f4")

        for i, (record_1, record_2) in enumerate(record_pairs):
            for fidx, compare, start, stop in self._field_comparators:
                val1 = record_1[fidx]
                val2 = record_2[fidx]
                if val1 is not None and val2 is not None:
                    distances[i, start:stop] = compare(val1, val2)
    ...
    def _add_derived_distances(self, distances: NpFloatArray) -> NpFloatArray:
    ...
    def check(self, record: Record) -> None:
        for fidx, _, _, _ in self._field_comparators:
            if fidx >= len(record):
                raise ValueError(
                    "Records do not line up with data model. "
                    "Data model contains a field with index '%s'" % fidx
                )
...
def typify_variables(
    ...
    for definition in variable_definitions:
        ...
        if variable_type == "FuzzyCategorical" and definition.other_fields is None:
            definition.other_fields = [  # type: ignore
                d.field_index
                for d in variable_definitions
                if d.field_index != definition.field_index
            ]
        ...
        try:
            ...
            raise KeyError(
                "Field type %s not valid. Valid types are %s"
    ...
    if only_custom:
        raise ValueError(
            "At least one of the variable types needs to be a type other "
            "than 'Custom'. 'Custom' types have no associated blocking rules"
        )
...
def missing(variables: list[Variable]) -> list[MissingDataType]:
    return [MissingDataType(var.name) for var in variables if var.has_missing]
...
def interaction_indices(variables: list[Variable]) -> list[list[int]]:
    ...
    for var in variables:
        if hasattr(var, "interaction_fields"):
            indices.append([var_names.index(f) for f in var.interaction_fields])
    return indices
